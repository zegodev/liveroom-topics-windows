...
#pragma comment(lib, "./../Topics/ScreenRecorder/ScreenCapture/libs/x86/ZegoScreenCapture.lib")
...
ZGScreenCaptureSource::ZGScreenCaptureSource()
{
	capture_video_data_vec_.resize(MAX_VIDEO_BUF_LEN);
	zego_screencapture_reg_captured_frame_available_notify(&ZGScreenCaptureSource::OnScreenCaptureData, this);
	zego_screencapture_reg_capture_error_notify(&ZGScreenCaptureSource::OnScreenCaptureError, this);
...
void ZGScreenCaptureSource::OpenSource()
{
	ZGENTER_FUN_LOG
...
void ZGScreenCaptureSource::CloseSource()
{
	ZGENTER_FUN_LOG
	if (have_start_captured_)
	{
...
void ZGScreenCaptureSource::OnGetVideoData(std::shared_ptr<ZGScreenVideoData> & video_data)
{
	if (!have_start_captured_)
	{
		OpenSource();
...
void ZGScreenCaptureSource::OnScreenCaptureData(const char *data, uint32_t length, const struct ZegoScreenCaptureVideoCaptureFormat *video_frame_format, uint64_t reference_time, uint32_t reference_time_scale, void *user_data)
{
	ZGScreenCaptureSource * pthis = (ZGScreenCaptureSource*)user_data;

	if (video_frame_format->video_pixel_format == kZegoPixelFormatBGRA32)
	{
		std::shared_ptr<ZGScreenVideoData> external_data = std::make_shared<ZGScreenVideoData>();
...
void ZGScreenCaptureSource::OnScreenCaptureError(enum ZegoScreenCaptureCaptureError error, void *data)
{
	ZGScreenCaptureSource * pthis = (ZGScreenCaptureSource*)data;

	ZGLog("ZGScreenCaptureSource::OnScreenCaptureError, %d", error);
}
